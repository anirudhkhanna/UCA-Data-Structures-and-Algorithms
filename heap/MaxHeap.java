...
public class MaxHeap {
	...
	private int arr[];	// array representation of the heap (index of left & right child: 2i and 2i+1, when indexing starts from 1) (index of left & right child: 2i+1 and 2i+2, when indexing starts from 0)
	...
	public MaxHeap(int size) {
		...
		this.pos = 0; // initially a place behind the first usable position (would be -1 in 0-based indexing)
	}
	...
	private void sinkDown(int i) {
		...
		// check if any of the children are larger than the ith element. Set max to the largest of ith element and its children, if any.
		...
		// if largest is not root (ith element) anymore, then we must swap it and further sink it
		if(max != i) {
			swap(i, max);
	...
	public void heapSort(int list[]) {
		...
		// build a heap by rearranging the array (from half of the array to its beginning, not taking leaf nodes unnecessarily)
